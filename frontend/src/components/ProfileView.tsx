...
export const ProfileView: React.FC<ProfileViewProps> = ({
  ...
  const userWorkspaces = (workspaces || []).filter(ws => 
    ws && (ws.ownerId === currentUser.id || (Array.isArray(ws.members) && ws.members.some(m => m.userId === currentUser.id)))
  );
            ...
            <div className="pt-4 border-t border-slate-800/80">
              ...
              {userWorkspaces.length === 0 ? (
                <div className="p-6 bg-slate-900/60 border border-slate-800 rounded-2xl text-center">
                  <div className="w-10 h-10 mx-auto rounded-xl bg-slate-800/80 border border-slate-700/80 flex items-center justify-center text-slate-400 mb-2.5">
                    <Lock size={18} />
                  </div>
                  <h4 className="text-xs font-semibold text-slate-200 mb-1">No Workspace Memberships</h4>
                  <p className="text-[11px] text-slate-400 max-w-sm mx-auto">
                    Under Zero-Trust organization policy, your account is not currently assigned to any workspace.
                  </p>
                </div>
              ) : (
