...
export const App: React.FC = () => {
  ...
  // ---------------------------------------------------------------------------
  // Strict Zero-Trust Workspace Access Resolution: User ONLY accesses workspaces they own or are members of
  const accessibleWorkspaces = useMemo(() => {
    if (!currentUser || !currentUser.id) return [];
    return workspaces.filter(w => {
      if (!w) return false;
      const isOwner = w.ownerId === currentUser.id;
      const isMember = Array.isArray(w.members) && w.members.some(m => m.userId === currentUser.id);
  ...
  const checkGatewayHealth = async (targetWsId?: string) => {
    ...
    if (syncResult.isBackendConnected) {
      ...
      if (syncResult.workspaces !== undefined) {
        setWorkspaces(syncResult.workspaces);
      }
      if (syncResult.boards !== undefined) {
        setBoards(syncResult.boards);
      }
      if (syncResult.teams !== undefined) {
  ...
  const handleSuccessfulAuth = (user: Assignee, preferredWsId?: string, token?: string) => {
    ...
    // Strict Zero-Trust: Determine if user actually has access to the target workspace or any workspace
    const userWorkspaces = workspaces.filter(w => 
      w.ownerId === user.id || 
      (Array.isArray(w.members) && w.members.some(m => m.userId === user.id))
    );

    const validTargetWs = (preferredWsId && userWorkspaces.some(w => w.id === preferredWsId))
      ? preferredWsId
      : (userWorkspaces.length > 0 ? userWorkspaces[0].id : '');

    if (validTargetWs) {
      setCurrentWorkspaceId(validTargetWs);
      const wsBoards = boards.filter(b => b.workspaceId === validTargetWs);
