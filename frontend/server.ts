...
function ensureUserWorkspaceAndBoard(user: Assignee) {
  // Respect unaffiliated users who have no workspace memberships
  if (user.id === 'user-6' || (Array.isArray(user.workspaceIds) && user.workspaceIds.length === 0)) {
    return;
  }
  let ws = workspaces.find(w => w.ownerId === user.id || (w.members && w.members.some(m => m.userId === user.id)));
  if (!ws) {
    // Under Zero-Trust, do not synthesize workspaces unless user explicitly registers or creates one
    return;
  }

  // Ensure workspaceIds only contains workspaces where user is owner or member
  const allUserWorkspaces = workspaces.filter(w => 
    w.ownerId === user.id || (w.members && w.members.some(m => m.userId === user.id))
  );
  user.workspaceIds = allUserWorkspaces.map(w => w.id);
...
function authenticateUser(req: Request): Assignee | null {
  ...
  // 5. Fallback for explicit demo tokens
  if (token === "demo-token" || token === "demo") {
    const demo = users.find(u => u.id === 'user-1') || null;
    return demo;
  }

  // Strict Zero-Trust: If user cannot be authenticated, NEVER return users[0] by default!
  return null;
}
